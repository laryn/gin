((Backdrop, once, { computePosition, offset, arrow, shift, flip }) => {
  /**
   ...
   *
   * @todo once we move into the Backdrop core menu system, we should be able to
   * remove this.
   */
  ...
  function expandCollapseSidebar(toState) {
    ...
    sidebar.querySelector('#sidebar-state').textContent = toState ? Backdrop.t('Collapse sidebar') : Backdrop.t('Expand sidebar');
    expandCollapseButton.setAttribute('aria-expanded', toState);
    localStorage.setItem('Backdrop.navigation.sidebarExpanded', toState);
  ...
  function closeAllSubmenus(Element) {
    ...
    submenuParentElement.querySelectorAll(`.toolbar-link[aria-expanded="true"]:not(:is(${selectorsToIgnore}))`).forEach(el => {
      ...
      el.querySelector('.toolbar-link__action').textContent = Backdrop.t('Extend');
    });
  }
  ...
  function openCloseSubmenu(parentListItem, state) {
    ...
    buttonEl.querySelector('.toolbar-link__action').textContent = toState ? Backdrop.t('Extend') : Backdrop.t('Collapse');
    // Gin Custom start ---------------------
    // checkOverflow();
    // Gin Custom end ------------------------
  }
  ...
  /**
   * Get the first focusable element in the page content (not Backdrop toolbar).
  ...
  function init(el) {
    ...
    expandCollapseSidebar(localStorage.getItem('Backdrop.navigation.sidebarExpanded') !== 'false');
    initDisplace();
